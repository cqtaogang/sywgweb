   ...
   static boolean feeflag=false;
    public static Connection getConnection()
	{
		...
		try
		{
			...
			con=DriverManager.getConnection("jdbc:mysql://58.17.241.16:3306/bookstore?useUnicode=true&characterEncoding=UTF-8","book","book123");  
		}
		catch(Exception e)
		{
			e.printStackTrace();
    ...
    public static int CheckPermitted(String mgNO)
    {
    	...
    	try
    	{
    		Connection con=getConnection();
        	Statement st=con.createStatement();
        	String sql="select S_Permitted from student where S_Num='"+mgNO+"'";
        	ResultSet rs=st.executeQuery(sql);
        	if(rs.next())
        	{
